  ...
  User.find({})
    ...
    .catch((err) =>
    {
      return err.name === 'ValidationError'
        ? res.status(UNCORRECT_DATA_ERROR_CODE).send({ message: "Ошибка получения пользователей" })
...
module.exports.createUser = (req, res) => {
  ...
  User.create({ name, about, avatar })
    ...
    {
      return err.name === 'ValidationError'
        ? res.status(UNCORRECT_DATA_ERROR_CODE).send({ message: "Переданы некорректные данные при создании пользователя" })
        : res.status(DEFAULT_ERROR_CODE).send({ message: "Ошибка по умолчанию" });
    });
};
  ...
  User.findByIdAndUpdate(req.user._id, { name: name, about: about })
    ...
    .catch((err) =>
    {
      if (err.name === 'ValidationError'){
        return res.status(UNCORRECT_DATA_ERROR_CODE).send({ message: "Переданы некорректные данные при обновлении профиля" });
...
module.exports.updateUserAvatar = (req, res) => {
  ...
  User.findByIdAndUpdate(req.user._id, { avatar: avatar })
    ...
    {
      if (err.name === 'ValidationError'){
        return res.status(UNCORRECT_DATA_ERROR_CODE).send({ message: "Переданы некорректные данные при обновлении аватара" });
      } else if(err.name === 'CastError'){
        return res.status(NOT_FOUND_ERROR_CODE).send({ message: "Пользователь с указанным _id не найден" })
      } else{
        return res.status(DEFAULT_ERROR_CODE).send({ message: "Ошибка по умолчанию" });
